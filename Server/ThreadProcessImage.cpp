...
ThreadProcessImage::ThreadProcessImage() {
  ...
  string modelPath =
      emotiEffLibRootDir + "/models/affectnet_emotions/onnx/" + modelName + ext;
  ...
  string filepath = std::filesystem::current_path() /
                    "mediapipe_addition/graph_strings/face_cpu.txt";
  string graph_string = LoadFileToString(filepath);
  libmp_face.reset(
      mediapipe::LibMP::Create(graph_string.c_str(), "input_video"));
  ...
  filepath = std::filesystem::current_path() /
             "mediapipe_addition/graph_strings/hand_cpu.txt";
  ...
  filepath = std::filesystem::current_path() /
             "mediapipe_addition/graph_strings/pose_cpu.txt";
  graph_string = LoadFileToString(filepath);
  libmp_pose.reset(
      mediapipe::LibMP::Create(graph_string.c_str(), "input_video"));
  ...
  string InspireFaceModelPath =
      Homepath + "/RobotNurseHelper_build/InspireFace/test_res/pack/Pikachu";
  ...
  HFDetectMode detectMode =
      HF_DETECT_MODE_TRACK_BY_DETECTION; // HF_DETECT_MODE_ALWAYS_DETECT;
                                         // //HF_DETECT_MODE_LIGHT_TRACK;
                                         // //HF_DETECT_MODE_TRACK_BY_DETECTION;
                                         // //
                                         // 使用偵測驅動的追蹤模式，適合高解析度視頻流
  ...
  HInt32 trackByDetectModeFPS =
      -1; // if MODE_TRACK_BY_DETECTION, default value -1 means 30fps
  HResult sessionRet = HFCreateInspireFaceSession(
      customParameter, detectMode, maxDetectFaceNum, detectPixelLevel,
    ...
    if (DataFrames_queue.size() > 0) {
      ...
      if (bCorrectlyDecoded) {
        ...
        if (bSaveTransmittedImage) {
          ...
          if (iFrameCount == 0 ||
              protobufTimestamp - previous_image_save_time >
                  chrono::milliseconds(
                      msetting.iImageSaveIntervalMillisecond)) {
            const bool bMillisecond = true;
            mstr_captured_timestamp = ConvertProtobufTimestampToString(
                RTSmessage.event_time(),
                bMillisecond); // use event_time as the timestamp for saving
                               // images. It is more accurate than using the
                               // current time.
            string filename =
                ImageSaveDirectory + "/" + mstr_captured_timestamp + ".jpg";
        ...
        if (bSaveRequestedPhoto) {
          ...
          const bool bMillisecond = true;
          mstr_captured_timestamp = ConvertProtobufTimestampToString(
              RTSmessage.event_time(), bMillisecond);
          ...
          string filename = ImageSaveDirectory_VisualCompass + requestedPhotoPrefix +
                            "_" + mstr_captured_timestamp + ".jpg";
        ...
        if (msetting.bHumanPoseEstimation) {
          ...
          if (msetting.PoseEstimationModel == "Yolo11n_Pose") {
            ...
            cout << "Unknown PoseEstimationModel: "
                 << msetting.PoseEstimationModel
          ...
          if (use_Yolo11n_Pose) {
            NL_pose =
                yolo11pose.Process(inputImage); // process the inputImage and
          ...
          if (use_Yolo11n_Pose) {
            ...
            for (int pose_num = 0; pose_num < num_poses; pose_num++) {
              ...
              for (auto &pr : yolo11pose.skeleton) {
                ...
                if (a < num_kps && b2 < num_kps && kp_conf_a > KP_CONF_THRES &&
                  ...
                  int a_x = static_cast<int>(NL_pose[pose_num][a][0] *
                                             inputImage.cols);
                  int a_y = static_cast<int>(NL_pose[pose_num][a][1] *
                                             inputImage.rows);
                  int b2_x = static_cast<int>(NL_pose[pose_num][b2][0] *
                                              inputImage.cols);
                  int b2_y = static_cast<int>(NL_pose[pose_num][b2][1] *
                                              inputImage.rows);
                  cv::line(outFrame, cv::Point(a_x, a_y), cv::Point(b2_x, b2_y),
                           col, 3, cv::LINE_AA);
                }
              }
            }
            ...
            if (libmp_pose->WriteOutputImage(
                    outFrame.data,
                    libmp_pose->GetOutputPacket("output_video"))) {
              bNewoutFrame = true;
            } else {
              cout << "WriteOutputImage fails." << std::endl;
            }

            NL_pose = get_landmarks_pose(
                libmp_pose); // I use this to guide robot's movement
          }
        }
        ...
        if (msetting.bHandLandmarkDetection) {
          ...
          if (libmp_hand->WriteOutputImage(
                  tempFrame.data,
                  libmp_hand->GetOutputPacket("output_video"))) {
            bNewoutFrame = true;
          } else {
          ...
          if (bDrawImageByOurOwn) {
            ...
            for (int hand_num = 0; hand_num < num_hands; hand_num++) {
              for (const std::array<float, 3> &norm_xyz : NL_hands[hand_num]) {
                ...
                cv::circle(outFrame, cv::Point(x, y), 5, cv::Scalar(0, 0, 255),
                           -1);
              }
            }
          }
        ...
        if (msetting.bFaceDetection) {
          if (msetting.FaceDetectionModel == "InspireFace") {
            ...
            result = HFCreateImageStreamFromImageBitmap(
                image, HF_CAMERA_ROTATION_0, &imageHandle);
            if (result != HSUCCEED) {
            ...
            result = HFSessionSetFaceDetectThreshold(
                session,
                0.5); // 可選：設定人臉偵測的信心閾值，範圍為 0.0 到 1.0
            if (result != HSUCCEED) {
              std::cerr << "Failed to set face detection threshold."
                        << std::endl;
              break;
            }
            ...
            result =
                HFExecuteFaceTrack(session, imageHandle, &MultipleFaceData);
            if (result != HSUCCEED) {
            ...
            if (MultipleFaceData.detectedNum > 0) {
              ...
              result = HFMultipleFacePipelineProcess(
                  session, imageHandle, &MultipleFaceData, customParameter);
              if (result != HSUCCEED) {
                std::cerr << "Failed to process multiple face pipeline."
                          << std::endl;
                break;
              }
              ...
              HResult result =
                  HFGetFaceAttributeResult(session, &faceAttributeResult);
              if (result == HSUCCEED) {
                string FaceGender = "";
                string FaceAge = "";
                for (int i = 0; i < faceAttributeResult.num; i++) {
                  ...
                  cv::rectangle(outFrame, cv::Point(faceRect.x, faceRect.y),
                                cv::Point(faceRect.x + faceRect.width - 1,
                                          faceRect.y + faceRect.height - 1),
                                cv::Scalar(0, 255, 0), 2);
                  // cout << "MultipleFaceData.trackIds[i]: " <<
                  // MultipleFaceData.trackIds[i] << endl;
                  cv::putText(outFrame,
                              std::to_string(MultipleFaceData.trackIds[i]) +
                                  " " + FaceGender + " " + FaceAge,
                ...
                cout << "Failed to get face attribute result, error code: "
                     << result << endl;
              }
            }
            ...
            if (libmp_face->WriteOutputImage(
                    tempFrame.data,
                    libmp_face->GetOutputPacket("output_video"))) {
              bNewoutFrame = true;
            } else {
              cout << "WriteOutputImage fails." << std::endl;
            }
            ...
            if (bDrawImageByOurOwn) {
              ...
              for (int face_num = 0; face_num < num_faces; face_num++) {
                for (const std::array<float, 3> &norm_xyz :
                     NL_faces[face_num]) {
                  int x = static_cast<int>(norm_xyz[0] * inputImage.cols);
                  int y = static_cast<int>(norm_xyz[1] * inputImage.rows);
                  cv::circle(outFrame, cv::Point(x, y), 1,
                             cv::Scalar(0, 255, 0), -1);
                }
              }
            }
            ...
            if (!NL_faces.empty()) {
              ...
              for (int face_num = 0; face_num < num_faces; face_num++) {
                ...
                if (msetting.bFacialExpressionRecognition) {
                  auto res = fer->predictEmotions(
                      face, false); // false will return the softmax scores
                  Rect roi = GetBoundingBoxFromLandmarks(
                      NL_faces[face_num], inputImage.cols, inputImage.rows);
                  ...
                  cv::putText(outFrame, text, Point(roi.x, roi.y),
                              cv::FONT_HERSHEY_SIMPLEX, 1.,
                              cv::Scalar(0, 255, 0), 2);
                }
                ...
                if (msetting.bUseDlibForFaceRecognition) {
                  ...
                  dlib::assign_image(dlib_face,
                                     dlib::cv_image<dlib::bgr_pixel>(face));
                  ...
                  cout << "face descriptor for one face: "
                       << dlib::trans(face_descriptors[0]) << endl;
                }
            ...
            cout << "Unknown FaceDetectionModel: "
                 << msetting.FaceDetectionModel
        ...
        if (bSaveProcessResult) {
          string filename = ImageSaveDirectory + "/" + mstr_captured_timestamp +
                            ".outFrame.jpg";
          ...
          for (int face_num = 0; face_num < num_faces; face_num++) {
            Mat face = CropRegion(inputImage, NL_faces[face_num]);
            filename = ImageSaveDirectory + "/" + mstr_captured_timestamp +
                       ".face" + to_string(face_num) + ".jpg";
                                ...
                                // code will not be executed.
          {
            ...
            if (duration.count() >= 1) {
              if (action_option.move_mode != action_option.MOVE_MANUAL) {
                ...
                PoseLandmarks_to_RobotAction_yolo(NL_pose, robot_status,
                                                  action_option, command);
                previous_time = current_time;
                pSendMessageManager->AddMessage(
                    command); // The command is filled in the
      ...
      if (bShowTransmittedImage) {
        ...
        std::cout << "Elapsed time: " << duration_ms.count() << " milliseconds"
                  << std::endl;
      }
...
void ThreadProcessImage::NotifyEvent(
    string description, chrono::time_point<chrono::system_clock> timestamp,
...
Mat ThreadProcessImage::CropRegion(
    Mat inputImage, std::vector<std::array<float, 3>> normalized_landmarks) {
  Rect roi = GetBoundingBoxFromLandmarks(normalized_landmarks, inputImage.cols,
                                         inputImage.rows);
  Mat cropped_face = inputImage(roi).clone(); // clone to ensure a deep copy
  return cropped_face;
}

cv::Rect ThreadProcessImage::GetBoundingBoxFromLandmarks(
    const std::vector<std::array<float, 3>> &normalized_landmarks,
